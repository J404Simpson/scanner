document.addEventListener('DOMContentLoaded', async () => {
  ...
  function populateConsignmentTable() {
    ...
    consignmentItems.forEach((item, index) => {
      ...
      row.innerHTML = `
        ...
        <td hidden data-label="Lot Number">${item.cr5bd_lotnumber || ''}</td>
  ...
  function addToTable(index, entry) {
    ...
    row.innerHTML = `
      ...
      <td hidden data-label="Lot Number">${scannedLot}</td>
  ...
  submitBtn.addEventListener('click', () => {
    ...
    const codes = rows.map(row => ({
      ...
      description: row.querySelector('[data-label="Description"]').textContent.trim(),
